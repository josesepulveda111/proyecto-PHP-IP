...
<div class="ui container">
    ...
    <a href="menu.php?mod=consultarsalidas" class="ui button btn_consultar">Consultar salidas</a>
    <form class="ui fluid form" action="salidas/registrar.php" method="POST">
        <br>
        <!-- <div class="field">
                <?php
            ...
            <label for="">Seleccione el proveedor de los productos si lo considera necesario</label>
        ...
        <div class="field">
                <?php
                // Sentencia SQL para consultar los supermercados
                $consultaSupermercado="SELECT * FROM tbl_supermercados where estado_cli='Activo'";
                // Obtenemos los resultadps
                $consultaResultado=mysqli_query($conexion, $consultaSupermercado);
                ?>
    
            <label for="">Supermercado</label>
            <select class="ui fluid search dropdown" name="supermercado" id="supermercado" required>
                <option value="">Seleccione el supermercado</option>

                <?php
                // Desde que haya resultados, los mostramso
                while($consultaS=mysqli_fetch_array($consultaResultado)){ ?>

                  <option value="<?php echo $consultaS['rut_nit_cli']; ?>"><?php echo $consultaS['nombre_cli']; ?></option>
        ...
        <tr>
            <th colspan="7">TOTAL SALIDA: $<span id="totalFactura">0</span></th>
            <input type="hidden" name="totalSalida" id="totalFacturaInput">
        </tr>
        <br>
    </table>
        <div class="centrarboton">
            <input type="submit" name="btn_insertar" value="Guardar salida" class="positive ui button">
...
<script>
    ...
    // Esta función se hace con el fin de que carguen los selects sobre el primer select
    function cargarProductos1(){
        let idproductos="idproductos1";
        var content=document.getElementById(idproductos);
        // var formData= new FormData();
        // formData.append('idproveedor', proveedor);
        
        fetch("salidas/loadselect.php", {
            method: "POST"
    ...
    // Se llama la función una vez cargue el archivo
    cargarProductos1();
